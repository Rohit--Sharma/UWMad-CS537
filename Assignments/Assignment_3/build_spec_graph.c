...
void add_dependency(struct directed_acyclic_graph* dag, MakeNode* target, MakeNode* dependency)
{
    ...
    struct graph_adj_list_node* target_node = new_adj_list_node(target);
    struct graph_adj_list_node* dependency_node = new_adj_list_node(dependency);
    int i;
    int src_node = -1;
    int dest_node = -1;

    for (i=0; i<(dag->targets_and_dependencies); i++) {
            if (dag->dependencies[i].head == target_node) {
        	    src_node = i;
        	    continue;
            }
            if (dag->dependencies[i].head == dependency_node) {
        	    dest_node = i;
        	    continue;
            }
    }
    ...
    dag->dependencies[src_node].head = dependency_node;
}
